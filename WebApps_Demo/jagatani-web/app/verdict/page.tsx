...
export default function VerdictPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 max-w-md mx-auto flex flex-col items-center justify-center space-y-5 pb-24">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-slate-500 text-sm font-medium">AI is analyzing your receipt</p>
        </div>
        <BottomNav />
      </div>
    );
  ...
  if (!result) {
    return (
      <div className="min-h-screen bg-slate-50 max-w-md mx-auto flex flex-col items-center justify-center space-y-4 pb-24">
        <p className="text-slate-600 font-semibold text-sm">No audit data found. Go back to Home.</p>
  ...
  const verdictConfig: Record<VerdictState, { bg: string; border: string; icon: any; iconColor: string; iconBg: string; label: string; labelColor: string; labelBg: string; desc: string; subDesc: string }> = {
    ACCEPTED: {
      ...
      desc: "Quality passed", subDesc: "You earned from this upload.",
    },
    RETAKE: {
    ...
    REJECTED: {
      ...
      desc: "No payment", subDesc: "This image cannot be used as a payment.",
    },
  };
  ...
  const handleSpeakAloud = () => {
    if (!audioSrc) { alert("No audio brief available for this receipt."); return; }
    if (isPlayingAudio) { setIsPlayingAudio(false); return; }
    const audio = new Audio(audioSrc);
    audio.play().catch(() => alert("Could not play audio. Check API server."));
  ...
  return (
    <div className="min-h-screen bg-slate-50 max-w-md mx-auto pb-24 font-sans">
      <header className="bg-white border-b border-slate-100 px-5 pt-12 pb-4">
        ...
        <p className="text-slate-500 text-xs font-medium mt-0.5">Shown immediately after capture</p>
      </header>
      ...
      <main className="px-4 pt-4 space-y-4">
        ...
        {record.merchant_name && (
          <div className="bg-white rounded-2xl px-4 py-3 border border-slate-100 shadow-xs flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-medium">{record.primary_receipt_category || record.primary_category || "Farm Receipt"}</p>
            </div>
          </div>
        )}
        ...
        <div className={"rounded-3xl p-5 border " + cfg.bg + " " + cfg.border}>
          ...
          {(["ACCEPTED", "RETAKE", "REJECTED"] as VerdictState[]).map((state) => {
            ...
            return (
              <div key={state} className={"flex items-start space-x-3 py-2.5 px-3 rounded-2xl mb-2 transition-all " + (isActive ? c.bg + " " + c.border + " border" : "opacity-35")}>
        ...
        <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-xs">
          <div className="flex items-center justify-between mb-3">
            ...
            >
              <Volume2 size={14} />
              <span>{isPlayingAudio ? "Playing..." : "Speak aloud"}</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="p-4 rounded-2xl text-center bg-slate-50 border border-slate-200">
              ...
              <p className="text-2xl font-black mt-1 text-slate-800">
                Rp {(financials.total_production_cost || record.total_production_cost_idr || 0).toLocaleString("en-US")}
              </p>
            </div>
          </div>
          ...
          {voice.transcript && (
            <div className="mt-3 bg-purple-50/80 rounded-2xl px-4 py-3 border border-purple-100">
              <p className="text-[10px] text-purple-700 font-bold uppercase tracking-wider mb-1">Voice Brief</p>
          ...
          <div className="bg-white rounded-3xl p-5 border border-slate-100 shadow-xs space-y-2">
            ...
            <div className="space-y-2 pt-1">
              <div className="flex items-center justify-between py-2.5 border-b border-slate-100">
                ...
                <p className="font-extrabold text-slate-900">Rp {(financials.total_production_cost || 0).toLocaleString("en-US")}</p>
              </div>
              <div className="flex items-center justify-between py-2.5 border-b border-slate-100">
            ...
            {verdictState === "RETAKE" ? (
              <>
                <button className="w-full border border-slate-200 text-slate-700 py-3 rounded-2xl font-semibold text-sm flex items-center justify-center space-x-2 hover:bg-slate-50">
                  <HelpCircle size={16} /><span>How to take a good receipt photo</span>
                </button>
              ...
              <>
                <div className="bg-red-50 border border-red-200 rounded-2xl p-4 space-y-2">
                  <div className="flex items-center space-x-2"><AlertTriangle size={16} className="text-red-600 shrink-0" /><p className="font-extrabold text-red-900 text-sm">No payment made</p></div>
                ...
                <label className="w-full bg-slate-900 text-white py-3.5 rounded-2xl font-extrabold text-sm hover:bg-slate-800 active:scale-[0.99] transition-all shadow-lg flex items-center justify-center space-x-2 cursor-pointer">
                  <Camera size={18} /><span>Try another photo</span>
                  <input type="file" accept="image/*" capture="environment" className="hidden"
                    onChange={() => { sessionStorage.removeItem("auditResult"); router.push("/home"); }} />
                </label>
              </>
