# TaniJaga Production Deployment Guide 🚀

This guide covers production deployment for the complete **TaniJaga** ecosystem:
...
- **AI Pipelines**: Google Gemini 2.5 Flash Vision OCR & ElevenLabs Audio Briefs
...
3. This creates:
   ...
   - `farmer_ledger` user isolation (`user_id`, `voice_transcript`, `audio_url`, `commodity`)
...
| `ALLOWED_ORIGINS` | Comma-separated CORS origins | `https://tanijaga.com,https://your-app.netlify.app` |
...
| `NEXT_PUBLIC_API_URL` | Public URL of the FastAPI Backend | `https://api.tanijaga.com` |

---

## 3. Deployment Option A: Cloud Hosting (Netlify + Render Setup)

### Deploy Frontend to Netlify
1. Push your code to GitHub (branch `feature/multi-commodity-enhancements` or `main`).
...
- [ ] Toggle between commodities (Corn, Chili, Rice) and verify persistent selection.
- [ ] Upload a test receipt photo and verify:
  - Gemini OCR audit runs successfully.
  - Break Even Point (BEP) is calculated with crop-specific yield.
...
- [ ] Check `/report` for the user's verified financial recap and KUR credit report.
